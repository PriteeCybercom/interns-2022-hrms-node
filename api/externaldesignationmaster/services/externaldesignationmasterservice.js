...
module.exports.getdata = async () => {
  return new Promise(async (resolev, reject) => {
    framework.db.externaldesignationmaster
...
module.exports.postdata = async (bodydata) => {
  return new Promise(async (resolev, reject) => {
    framework.db.externaldesignationmaster.findOne({ where: { title: bodydata.title, deletedDate: null} })
      .then((data) => {
        if (data == null) {
          framework.db.externaldesignationmaster
            .create(
...
module.exports.putdata = async (bodydata, id) => {
  return new Promise(async (resolve, reject) => {
    framework.db.externaldesignationmaster.findOne({ where: { title: bodydata.title, deletedDate: null ,id:{[Op.ne]:id}}}).then((data)=>{
      if(data==null){
        framework.db.externaldesignationmaster
        .update(
